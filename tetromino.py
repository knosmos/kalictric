...
# Kalictric support by Jieruei Chang
# Released under a "Simplified BSD" license

# KRT 17/06/2012 rewrite event detection to deal with mouse use

import random, time, pygame, sys, kmodule, mido
...
def main():
    global FPSCLOCK, screen, BASICFONT, BIGFONT, WINDOWWIDTH, WINDOWHEIGHT, musicthread
    pygame.init()
    ...
    musicthread = Thread(target = midiThread)
...
def runGame():
    ...
    while True: # game loop
        ...
        if constdetection == 7 and isValidPosition(board, fallingPiece, adjX = -1):
            fallingPiece['x'] -= 1
            movingLeft = True
            movingRight = False
            lastMoveSidewaysTime = time.time()
        if constdetection == 10 and isValidPosition(board, fallingPiece, adjX = 1):
            fallingPiece['x'] += 1
            movingRight = True
            movingLeft = False
            lastMoveSidewaysTime = time.time()
        ...
        for event in pygame.event.get(): # event handling loop
            if event.type == KEYUP:
                ...
                if (event.key == K_LEFT or event.key == K_a or detection == 6) and isValidPosition(board, fallingPiece, adjX=-1):
                    fallingPiece['x'] -= 1
                    movingLeft = True
                    movingRight = False
                    lastMoveSidewaysTime = time.time()

                elif (event.key == K_RIGHT or event.key == K_d or detection == 9) and isValidPosition(board, fallingPiece, adjX=1):
                    fallingPiece['x'] += 1
                    movingRight = True
                    movingLeft = False
                    lastMoveSidewaysTime = time.time()
                ...
                elif (event.key == K_DOWN or event.key == K_s or detection == 8):
                    movingDown = True
                    if isValidPosition(board, fallingPiece, adjY=1):
                        fallingPiece['y'] += 1
                    lastMoveDownTime = time.time()
